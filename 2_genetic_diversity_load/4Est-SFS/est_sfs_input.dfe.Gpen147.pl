#!/usr/bin/perl
##usage est_sfs_input.dfe.pl [freq file of specific species][outgroup1][outgroup2]
#
use strict;
use File::Basename;
...
while(<IN>){
	...
	my @allele1=split(/:/,$line[4]);
        ...
        if ($alle1 =~ "A"){$a=sprintf("%.0f",294*$alle1_c);}
        if ($alle1 =~ "T"){$t=sprintf("%.0f",294*$alle1_c);}
        if ($alle1 =~ "C"){$c=sprintf("%.0f",294*$alle1_c);}
        if ($alle1 =~ "G"){$g=sprintf("%.0f",294*$alle1_c);}

        my @allele2=split(/:/,$line[5]);
        my $alle2=$allele2[0];
        my $alle2_c=$allele2[1];

        if ($alle2 =~ "A"){$a=sprintf("%.0f",294*$alle2_c);}
        if ($alle2 =~ "T"){$t=sprintf("%.0f",294*$alle2_c);}
        if ($alle2 =~ "C"){$c=sprintf("%.0f",294*$alle2_c);}
        if ($alle2 =~ "G"){$g=sprintf("%.0f",294*$alle2_c);}
...
while(<OUT2>){
	...
	if (exists $snps{$snp_id}){
		...
		my $snps_t=0;
	        ...
	        if ($alle1 =~ "A"){$snps_a=294;}
	        if ($alle1 =~ "C"){$snps_c=294;}
        	if ($alle1 =~ "G"){$snps_g=294;}
        	if ($alle1 =~ "T"){$snps_t=294;}
